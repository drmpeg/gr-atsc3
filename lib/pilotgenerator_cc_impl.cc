...
namespace gr {
  namespace atsc3 {
                       ...
                       gr_vector_void_star &output_items)
    {
      ...
      gr_complex* dst;
      gr_complex* src;
      float angle, anglefft;
      gr_complex temp;
      int miso_left_nulls, miso_right_nulls;
      ...
      for (int i = 0; i < noutput_items; i += insertion_items) {
        for (int j = 0; j < symbols; j++) {
          ...
          if (miso_mode == MISO_64) {
            if (frame_symbols[j] != PREAMBLE_SYMBOL) {
              miso_left_nulls = ((ofdm_fft_size - carriers) / 2) + 1;;
              miso_right_nulls = (ofdm_fft_size - carriers) / 2;
              dst = miso_fft.get_inbuf();
              memcpy(&dst[0], &miso_coefficients_64_1_2[0], sizeof(gr_complex) * 64);
              std::fill_n(&dst[64], ofdm_fft_size - 64, 0);
              miso_fft.execute();
              src = miso_fft.get_outbuf();
              memcpy(&unfold[0], &src[ofdm_fft_size / 2], sizeof(gr_complex) * ofdm_fft_size / 2);
              memcpy(&unfold[ofdm_fft_size / 2], &src[0], sizeof(gr_complex) * ofdm_fft_size / 2);
              out -= carriers;
              for (int n = 0; n < carriers; n++) {
                anglefft = std::arg(unfold[n + miso_left_nulls]);
                if (j == 2 && n < 10)
                  printf("%f, ", anglefft);
                angle = miso_angle[n];
                if (angle > M_PI) {
